...
class SWAE3DEncoder(nn.Module):
    """3D SWAE Encoder following paper specifications"""
    def __init__(self, channels=[32, 64, 128], latent_dim=16):
        super(SWAE3DEncoder, self).__init__()
        
        # Input: 8x8x8x1 -> First conv block
        self.conv_blocks = nn.ModuleList()
        
        # First block: 1 -> 32 channels
        self.conv_blocks.append(Conv3DBlock(1, channels[0], stride=2))  # 8x8x8 -> 4x4x4
...
class SWAE3DDecoder(nn.Module):
    """3D SWAE Decoder following paper specifications"""
    def __init__(self, channels=[128, 64, 32], latent_dim=16):
        super(SWAE3DDecoder, self).__init__()
        ...
        self.deconv_blocks.append(DeConv3DBlock(channels[2], 1, stride=2, final_layer=True))  # 4x4x4 -> 8x8x8
...
class SWAE3D(nn.Module):
    """
    Pure SWAE 3D Autoencoder for Scientific Data Reconstruction
    ...
    def __init__(self, channels=[32, 64, 128], latent_dim=16, lambda_reg=10.0):
        super(SWAE3D, self).__init__()
        
        self.encoder = SWAE3DEncoder(channels, latent_dim)
        self.decoder = SWAE3DDecoder(list(reversed(channels)), latent_dim)
...
def create_swae_3d_model(block_size=8, latent_dim=16, lambda_reg=10.0):
    """
    Create SWAE 3D model with paper specifications
    
    Args:
        block_size: Size of 3D blocks (8x8x8 as per paper)
...
if __name__ == "__main__":
    ...
    # Test with dummy data (batch of 8x8x8 blocks)
    batch_size = 4
    ...
    print("\nSWAE 3D model created successfully!") 
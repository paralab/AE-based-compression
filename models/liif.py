    ...
    def query_rgb(self, coord, cell=None, scale=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # Use explicit scale information if provided, otherwise estimate from cell size
                if scale is not None:
                    # Use provided scale: t = S²
                    bs, q = coord.shape[:2]
                    if scale.dim() == 1:  # (batch_size,)
                        scale_squared = (scale ** 2).unsqueeze(1).expand(bs, q).unsqueeze(-1)  # (bs, q, 1)
                    else:  # Already expanded
                        scale_squared = scale ** 2
                        if scale_squared.dim() == 2:
                            scale_squared = scale_squared.unsqueeze(-1)
                else:
                    # Fallback: estimate from cell size
                    cell_area = torch.abs(cell[:, :, 0] * cell[:, :, 1])  # (batch_size, num_queries)
                    scale_squared = torch.clamp(4.0 / (cell_area + 1e-8), min=1.0, max=256.0)  # t = S²
                    scale_squared = scale_squared.unsqueeze(-1)  # (batch_size, num_queries, 1)

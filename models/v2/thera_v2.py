...
@register('thera')
class TheraNet(nn.Module):
    """
    Thera Neural Heat Field implementation
    ...
    def __init__(self, in_dim, out_dim, hidden_dim=256, num_frequencies=64, kappa=1.0):
        ...
        # Input: features from encoder + relative coordinates + cell info
...
@register('thera-simple')
class TheraNetSimple(nn.Module):
    """
    Simplified Thera implementation with fixed frequency bank
    """
    ...
    def __init__(self, in_dim, out_dim, hidden_dim=256, num_frequencies=64, kappa=1.0):
        ...
        self.register_buffer('W1', torch.randn(self.in_dim, num_frequencies) * 0.1)
        
        # MLP to predict output weights
    ...
    def forward(self, x):
        ...
        thermal_decay = torch.exp(-nu_squared * self.kappa * t_expanded)

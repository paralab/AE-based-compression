...
def measure_decompression_speed(model, latent_batch, device, num_warmup=5, num_trials=10):
    ...
    # For SWAE 7x7x7: output shape will be (batch_size, 1, 7, 7, 7)
    batch_size = latent_batch.shape[0]
    output_shape = (batch_size, 1, 7, 7, 7)  # Known output shape for SWAE 7x7x7
...
def benchmark_model_speed(model, eval_dataset, device, num_samples=50, batch_size=32):
    ...
    single_sample_bytes = calculate_data_size_bytes((1, 7, 7, 7))
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='test_u_chi_results',
    ...
    print("Loading U_CHI datasets for UNBIASED testing...")
    ...
    print("\n" + "="*50)
    print("🎯 FINAL VALIDATION METRICS (FIXED MODEL):")
    print("="*50)
    ...
    print(f"Expected PSNR: 14+ dB (Previous: 3.6 dB)")
    print(f"Expected Correlation: 0.85+ (Previous: 0.41)")
    print(f"PSNR Improvement: {max(0, avg_metrics['psnr'] - 3.6):.1f} dB gain")
    print(f"Correlation Improvement: {max(0, avg_metrics['correlation'] - 0.41):.3f} gain")
    print(f"Target Relative Error: <5% (Actual: {avg_metrics['mean_relative_error_percent']:.2f}%)")
    ...
    with open(metrics_file, 'w') as f:
        f.write("🎯 FIXED MODEL Validation Metrics:\n")
        f.write("="*40 + "\n")
        f.write("Model: SWAE 3D with FIXED per-sample pos_log normalization\n")
        ...
        f.write("-"*20 + "\n")
        f.write(f"Expected PSNR: 14+ dB (Previous: 3.6 dB)\n")
        f.write(f"Expected Correlation: 0.85+ (Previous: 0.41)\n")
        f.write(f"Actual PSNR Improvement: {max(0, avg_metrics['psnr'] - 3.6):.1f} dB gain\n")
        f.write(f"Actual Correlation Improvement: {max(0, avg_metrics['correlation'] - 0.41):.3f} gain\n")
        
        psnr_success = avg_metrics['psnr'] >= 14.0
        corr_success = avg_metrics['correlation'] >= 0.85
        rel_error_success = avg_metrics['mean_relative_error_percent'] <= 5.0
        
        f.write(f"\nSuccess Criteria Analysis:\n")
        f.write("-"*30 + "\n")
        f.write(f"PSNR ≥ 14 dB: {'✅ PASSED' if psnr_success else '❌ FAILED'} (Actual: {avg_metrics['psnr']:.2f} dB)\n")
        f.write(f"Correlation ≥ 0.85: {'✅ PASSED' if corr_success else '❌ FAILED'} (Actual: {avg_metrics['correlation']:.3f})\n")
        f.write(f"Mean Relative Error ≤ 5%: {'✅ PASSED' if rel_error_success else '❌ FAILED'} (Actual: {avg_metrics['mean_relative_error_percent']:.2f}%)\n")
        
        overall_success = psnr_success and corr_success and rel_error_success
        f.write(f"\nOverall Assessment: {'🎉 BREAKTHROUGH ACHIEVED' if overall_success else '⚠️ PARTIAL SUCCESS'}\n")
        
        if overall_success:
            f.write("All breakthrough criteria met! The FIXED per-sample normalization is working perfectly.\n")
        else:
            f.write("Some criteria not fully met. Further optimization may be needed.\n")

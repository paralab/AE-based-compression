...
def create_test_function(k1, k2, resolution=256):
    """Create a test mathematical function"""
...
def visualize_results(gt, lr, bicubic, thera, liif, k1, k2, scale_factor, save_path):
    ...
    axes[1, 0].set_title('Thera (Neural Heat Fields)')
    axes[1, 0].axis('off')
...
def main():
    ...
    parser.add_argument('--output_dir', default='results/thera_inference', help='Output directory')
    parser.add_argument('--scale_factors', nargs='+', type=int, default=[2, 4, 8, 16], help='Scale factors to test')
    ...
    for scale_factor in args.scale_factors:
        ...
        for func_spec in args.test_functions:
            ...
            lr_size = hr_resolution // scale_factor
            lr_func = F.interpolate(gt_func.unsqueeze(0), size=(lr_size, lr_size), 
                                  mode='bilinear', align_corners=False).squeeze(0)
            
            print(f"    LR size: {lr_func.shape}, HR size: {gt_func.shape}")
            ...
            # Visualize results
            save_path = os.path.join(args.output_dir, f'comparison_k{k1}k{k2}_scale{scale_factor}x.png')
            visualize_results(gt_func, lr_func, bicubic_result, thera_result, liif_result,
                            k1, k2, scale_factor, save_path)
    
    # Save detailed results
    results_file = os.path.join(args.output_dir, 'detailed_results.txt')
    with open(results_file, 'w') as f:
        f.write("Detailed Results - Thera vs Bicubic vs LIIF\n")
        f.write("=" * 60 + "\n\n")
        
        for result in results:
            f.write(f"Scale: {result['scale']}×, Function: sin(2π{result['k1']}x)sin(2π{result['k2']}y)\n")
